...
// rsync -av src  dst/    ./src/* --> /root/dst/src/*
func (c *Client) Upload(local string, remote string, f string) (err error) {
	...
	if info.IsDir() {
		...
		return c.UploadDir(local, remote, f)
	}
...
func (c *Client) UploadDir(localDir string, remoteDir string, f string) (err error) {
	// defer func() {
	// 	if err != nil {
